...
router.post("/add", (req, res) => {
  ...
  Classification.find({ className }).then((data) => {
    if (data.length > 0) {
      res.send({ code: 500, msg: "分类已存在" });
    } else {
      Classification.insertMany({ className, classDesc })
        .then(() => {
...
router.post("/NoClassPage", (req, res) => {
  ...
  let query = {
    isType,
    $or: [{ name: { $regex: name } }, { CargoSpace: { $regex: CargoSpace } }],
  };
